...
type RouteName = "ContributionScreen" | "DebtTabScreen" | "Home" | "ActivityTabScreen" | "ProfileScreen"
export default function BottomTabs() {
          ...
          const TabBarLabel = ({ focused, color, name }: { focused: boolean, color: string, name: RouteName }) => {
                    const icons = {
                              ...
                              ProfileScreen: focused ? require('../../assets/icons/user-focused.png') : require('../../assets/icons/user.png'),
                    }
                    return    <View style={[styles.tab, { width: TAB_BAR_WIDTH }]}>
                              <Image source={icons[name]} style={[styles.icon, name == "ProfileScreen" && { width: 30, height: 30 }]} />
                    </View>
          }
          return (
                    <TopTab.Navigator initialRouteName="Home" tabBarPosition="bottom" screenOptions={{
                              ...
                              <TopTab.Screen name="ProfileScreen" component={ProfileScreen} options={{ tabBarLabel: props => <TabBarLabel {...props} name="ProfileScreen" /> }} />
                    </TopTab.Navigator>
          )
}
